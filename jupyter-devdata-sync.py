...
# huggingface_hub.upload_folder hard-blocks .git/ via DEFAULT_IGNORE_PATTERNS
# regardless of the ignore_patterns= argument. Escape before upload and
# unescape after restore so git repos inside the Jupyter workspace survive
# Space restarts.
_GIT_ESCAPE = "__dot_git__"


def _escape_git_dirs(root: Path) -> None:
    """Rename every .git dir inside *root* to __dot_git__ before upload_folder."""
...
def _unescape_git_dirs(root: Path) -> None:
    """Rename every __dot_git__ dir inside *root* back to .git after restore."""
    for dirpath, dirnames, _ in os.walk(root, topdown=False):
        for dirname in list(dirnames):
...
def sync_loop(api, rid: str):
    while True:
        ...
        try:
            ...
            # Escape .git → __dot_git__ so upload_folder doesn't hard-block them.
            _escape_git_dirs(tmp)
            upload_folder(
                folder_path=str(tmp),
                repo_id=rid,
                repo_type="dataset",
